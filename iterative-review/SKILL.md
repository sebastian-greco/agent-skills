...
description: Iterative code review loop — spawns a fresh reviewer subagent, triages feedback by severity, fixes issues with individual commits, and loops until clean. Pauses every 5 rounds for confirmation. Works with any coding agent (Claude Code, Codex, Cursor, Copilot, etc.).
---
...
> **oh-my-opencode users:** See [`iterative-review-omo`](../iterative-review-omo/SKILL.md) for a version with named agent routing (`oracle` reviewer + `hephaestus` implementer).
